...
//                <i> PLL feedback clock must be in the range of 16 MHz to 60 MHz
...
//                <i> PLL output clock must be in the range of 2 MHz to 60 MHz
//      </e>
//
...
void SystemInit(void)
{
  ...
  HT_WDT->PR = 0x35CA;
  HT_WDT->MR1 = ((HT_WDT->MR1 & 0xFFF) | (WDT_PRESCALER << 12));
  HT_WDT->MR0 = WDT_RELOAD | (WDT_RESET_ENABLE << 13) | (WDT_SLEEP_HALT << 14) | (0x0 << 16);
  HT_CKCU->APBCCR1 &= ~(0x1 << 4);
  #endif
#endif
  ...
  #if (DEINIT_ENABLE == 1)
  do {
    ResetBit_BB((u32)(&HT_RTC->CR), 3);                         /* disable LSE                              */
  } while (GetBit_BB((u32)(&HT_RTC->CR), 3));
  #endif
#endif
...
void SystemCoreClockUpdate(void)
{
  ...
  u32 PllFeedbackClockDiv = ((HT_CKCU->PLLCFGR >> 23) == 0) ? (16) : ((HT_CKCU->PLLCFGR >> 23) & 15UL);
