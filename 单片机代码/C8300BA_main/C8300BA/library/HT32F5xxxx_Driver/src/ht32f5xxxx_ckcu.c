...
#define CKCU_MASK_REFCLKSEL     ((u32)0x1 << CKCU_POS_REFCLKSEL)
...
void CKCU_PLLInit(CKCU_PLLInitTypeDef *PLL_InitStruct)
{
  ...
  HT_CKCU->PLLCFGR = PLL_InitStruct->CFG;
}
...
void CKCU_GetClocksFrequency(CKCU_ClocksTypeDef* CKCU_Clk)
{
  ...
  CKCU_Clk->ADC_Freq = (div == 7) ? ((CKCU_Clk->HCLK_Freq) / 3) : ((CKCU_Clk->HCLK_Freq) >> div); 
}
...
bool CKCU_HSIAutoTrimIsReady(void)
{
  ...
  {
    lower_bound = 7812 - 195;
    upper_bound = 7812 + 195;
  }
  else
  {
    lower_bound = 8000 - 200;
    upper_bound = 8000 + 200;
  }

  return ((HT_CKCU->HSIATCR >= lower_bound) && (HT_CKCU->HSIATCR <= upper_bound)) ? TRUE : FALSE;
  #else
