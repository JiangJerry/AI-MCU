...
void USART_DeInit(HT_USART_TypeDef* USARTx)
{
  ...
  #endif
  #if (LIBCFG_UART2)
  else if (USARTx == HT_UART2)
  {
    RSTCUReset.Bit.UART2 = 1;
  }
  #endif
  #if (LIBCFG_UART3)
  else if (USARTx == HT_UART3)
  {
    RSTCUReset.Bit.UART3 = 1;
  }
  #endif
...
void USART_Init(HT_USART_TypeDef* USARTx, USART_InitTypeDef* USART_InitStruct)
{
  ...
  if (USARTx == HT_USART0)
    ...
    USARTx->DLR = CKCU_GetPeripFrequency(CKCU_PCLK_UART2) / (u32)USART_InitStruct->USART_BaudRate;
  #endif
  #if (LIBCFG_UART3)
  else if (USARTx == HT_UART3)
    USARTx->DLR = CKCU_GetPeripFrequency(CKCU_PCLK_UART3) / (u32)USART_InitStruct->USART_BaudRate;
  #endif
}
...
void USART_SendData(HT_USART_TypeDef* USARTx, u16 Data)
{
  ...
  USARTx->DR = Data;
	while(!(USARTx->SR&USART_FLAG_TXC)) WDT_Restart();
}
